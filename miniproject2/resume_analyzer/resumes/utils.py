...
def process_resume(file):
    ...
    if not text:
        return {
            ...
            'feedback': {'skill_gaps': [], 'formatting': [], 'ats_keywords': []}
        }
...
def match_resume_to_job(resume, job):
    ...
    resume_experience = float(re.search(r'(\d+\.\d+|\d+)', resume_experience_str).group()) if re.search(r'(\d+\.\d+|\d+)', resume_experience_str) else 0.0
    ...
    experience_diff = resume_experience - job.required_experience
    if experience_diff >= 0:
        experience_score = min((resume_experience / job.required_experience) * 30, 30) if job.required_experience else 30
    else:
